...
import { saveConfigToFile } from "../config/common.js";
export const help = () => {
    return {
        name: "主动群聊",
        type: "passive",
...
export class autoReply extends plugin {
    ...
    isMaster(userId) {
        if (!config.autoReply.masterQQ || typeof config.autoReply.masterQQ !== 'string') {
            return false;
        }
        const masterList = config.autoReply.masterQQ.split(',')
            .map(qq => qq.trim())
            .filter(qq => qq !== '')
            .map(qq => parseInt(qq));
        return masterList.includes(userId);
    }

    async switchChatPreset(e) {
        if (!this.isMaster(e.user_id) && e.sender.role != "owner") {
            return e.reply("只有机器人主人或群主可以切换聊天预设");
        }
    ...
    async toggleAutoReply(e) {
        if (!this.isMaster(e.user_id)) {
            return e.reply("只有机器人主人可以开关主动群聊功能");
        }
        const newValue = !config.autoReply.useAutoReply;
        config.autoReply.useAutoReply = newValue;
        saveConfigToFile(config.autoReply, "autoReply.yaml");
        await e.reply(`主动群聊功能已${newValue ? "开启" : "关闭"}`);
    }
    /**
