...
function getNextYearFestivalDate(festivalName, currentFestivalDate) {
    ...
    if (lunarFestivals[festivalName]) {
        ...
        nextFestivalDate = new Date(nextFestivalDate.getYear(), nextFestivalDate.getMonth() - 1, nextFestivalDate.getDay());
    }
    else {
...
function daysUntilFestival(festivalName, today, festivalDate) {
    if (festivalDate < today) {
        const nextFestivalDate = getNextYearFestivalDate(festivalName, festivalDate);
...
function getLunarFestivalsDates(today) {
    ...
    return Object.fromEntries(Object.entries(lunarFestivals).map(([name, [month, day]]) => {
